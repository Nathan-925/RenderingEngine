...
public class CoolShape extends Shape3D {

	private static double[][] generatePoints(double[] point, int detail, double radius) {
		double[][] points = new double[detail*detail][];
		for(int i = 0; i < detail; i++) {
			for(int j = 0; j < detail; j++) {
				points[i*detail+j] = new double[] {point[0]+radius*Math.cos(Math.PI*2*j/detail)-radius*Math.sin(Math.PI*2*i/detail), point[1]+radius*2*Math.cos(Math.PI*2*i/detail), point[2]+radius*Math.sin(Math.PI*2*j/detail)-radius*Math.sin(Math.PI*2*i/detail)};
			}
		}
	...
	public CoolShape(double[] point, int detail, double radius, Color color) {
